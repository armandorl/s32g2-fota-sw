...
#define HSE_SRV_ID_ACTIVATE_PASSIVE_BLOCK   ((hseSrvId_t)0x00000018UL)  /**< @brief Application request to switch passive flash block area (only for HSE_B). */
 ...
 *  @note     Available only for HSE-H. Applicable only at boot-time (till HSE sets HSE_STATUS_INIT_OK).
 *            In BOOT_SEQ = 1, application should update the clock configuration and send this signal only AFTER HSE_STATUS_BOOT_OK flag is set. */
 ...
 *           @note: HSE will set this bit only when BOOT_SEQ = 1 and secure boot is configured (e.g. SMR, SHE, BSB).
 *                  This bit represents the status of booting phase that includes PRE_BOOT verification and cores un-gating, but NOT POST_BOOT phase.
 *                  HSE will signal the end of the POST_BOOT phase along with additional peripherals initialization via HSE_STATUS_INIT_OK flag. */
