...
image_custom =  {'releaseCounter':1,'hardwareIdentifier':'BCM','ecuIdentifier':'gateway','hardwareVersion':3,'installMethod':'abUpdate','imageFormat':'binary','isCompressed':'gzip','dependency':2}
...
def clean_slate(use_new_keys=False):
  ...
  print(LOG_PREFIX + 'METADATA_EXTENSION:'+ tuf.conf.METADATA_FORMAT)
  ...
  '''
  VNP_image_custom =  {'releaseCounter':1,'hardwareIdentifier':'/GATEWAY','ecuIdentifier':'/GATEWAY','hardwareVersion':1,'installMethod':'inPlace','imageFormat':'binary','isCompressed':'noCompress','dependency':2}
  #VNP
  add_target_to_imagerepo('demo/images/GATEWAY_APP_0.bin','GATEWAY_APP_0.bin',VNP_image_custom )
  add_target_to_imagerepo('demo/images/GATEWAY_Flash_driver.bin', 'GATEWAY_Flash_driver.bin',VNP_image_custom )

  VDS_image_custom =  {'releaseCounter':1,'hardwareIdentifier':'/PDC','ecuIdentifier':'/PDC','hardwareVersion':1,'installMethod':'inPlace','imageFormat':'binary','isCompressed':'noCompress','dependency':2}
  #VDS
  add_target_to_imagerepo('demo/images/PDC_firmware.bin','PDC_firmware.bin',VDS_image_custom)


  GPIS_image_custom =  {'releaseCounter':1,'hardwareIdentifier':'/BCM','ecuIdentifier':'/BCM','hardwareVersion':1,'installMethod':'inPlace','imageFormat':'binary','isCompressed':'noCompress','dependency':2}
  #GPIS
  add_target_to_imagerepo('demo/images/BCM_APP_V1.0.bin','BCM_APP_V1.0.bin',GPIS_image_custom )
  add_target_to_imagerepo('demo/images/BCM_Flash_driver.bin', 'BCM_Flash_driver.bin',GPIS_image_custom )
 
  print(LOG_PREFIX + 'Signing and hosting initial repository metadata')
  #write_to_live()
  
  VNP_image_custom2 =  {'releaseCounter':2,'hardwareIdentifier':'/GATEWAY','ecuIdentifier':'/GATEWAY','hardwareVersion':1,'installMethod':'inPlace','imageFormat':'binary','isCompressed':'noCompress','dependency':2}
  #VNP
  add_target_to_imagerepo('demo/images/GATEWAY_APP_2.bin','GATEWAY_APP_2.bin',VNP_image_custom2 )
  add_target_to_imagerepo('demo/images/GATEWAY_Flash_driver_2.bin', 'GATEWAY_Flash_driver_2.bin',VNP_image_custom2 )
  '''
...
def write_to_live():
  ...
  if demo.FOTA_S32G_SWITCH:
    ...
    image_metada_path = os.path.join(demo.IMAGE_REPO_DIR, 'metadata')
    #shutil.move(os.path.join(image_metada_path,'root.json'), os.path.join(image_metada_path,str(root_metadata_verison)+'.root.json'))
    shutil.move(os.path.join(image_metada_path,'targets.json'), os.path.join(image_metada_path,str(targrt_metadata_verison)+'.targets.json'))
    shutil.move(os.path.join(image_metada_path,'snapshot.json'), os.path.join(image_metada_path,str(snapshot_metadata_verison)+'.snapshot.json'))




def add_target_to_imagerepo(target_fname,filepath_in_repo,custom):
  """
  ...
  print(LOG_PREFIX + 'add_target_to_imagerepo:'+ target_fname + filepath_in_repo)
  ...
  repo.targets.add_target(destination_filepath,custom)
...
def host():
  ...
  command = ['python3','../M_http_server_30301.py']
...
def test_fw_download_image(file):
  print('download the test image to ECU')
  filepath = '/home/songyu/workplace/cs-s32g-gw/uptane_test/test_data/image/'
  image = filepath + file
  print (image)
...
def listen():
  ...
  server.register_function(write_to_live,
      'write_image_repo')    

  server.register_function(
      test_fw_download_image, 'download_image')
